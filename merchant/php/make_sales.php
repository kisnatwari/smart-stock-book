<?php 
	...
	$data["products_data"] = json_decode($_POST["products_data"], true);
		...
		if($data["email"] != null)
			if(!filter_var($data["email"], FILTER_VALIDATE_EMAIL))
				show_error_string("Invalid customer email");
	...
	if(!empty($data["saved"])){
		...
		if(!preg_match("/^[a-zA-Z\s]+$/",$data["name"]))
	   		show_error_string("Some of the characters used in name are considered invalid");

		//validating phone number
		if($data["contact"] != null){
			if(!preg_match("/^[+]?[1-9]{0,3}[\s|-]?[0-9]{10,12}$/", $data["contact"]))
				show_error_string("Unsupported contact number of customer..");
		}
		...
		if($data["address"] != null){
			...
			if(!preg_match("/^[a-zA-Z0-9\s,.+-]+$/",$data["address"]))
			    show_error_string("Invalid characters in address");
		}
