	<?php 
		...
		$all_stocks = $pdo -> prepare("
			SELECT products.id, categories.category, products.category_id, products.name, products.marked_price, products.photo, products.available_stock
...
if(count($stocks) > 0){
	echo "<script> var stocks = ".json_encode($stocks)."; console.log(stocks); </script>";
 ?>
	<style>
		.product-card:hover{
		...
		function show_products(){
			...
			if(mode == "grid"){
				for(var i = 0; i<stocks.length; i++){
					...
					<div class="product-card bg-white border m-2 p-2 pb-3 text-center sh position-relative" style="width: 240px; overflow: hidden">
					<input type="checkbox" class="product-checkbox position-absolute form-check-input" value ="${stocks[i]['id']}" style="right: 20px;" oninput="product_check()">
	                        ...
	                        <strong style="font-size: 13px;">On Stock : ${stocks[i]['available_stock']}</strong><br>
